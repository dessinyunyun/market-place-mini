...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const config = {
      ...
      url: `${process.env.BASE_URL}supplier/find-all?limit=${limit}&offset=${pagination}`,
      headers: {
  ...
  const deleteSupplier = async (id, namasupplier) => {
    const result = confirm(`Apakah Anda yakin ingin menghapus ${namasupplier}?`);
    if (result) {
      const token = localStorage.getItem("token");
      try {
        const config = {
          method: "delete",
          maxBodyLength: Infinity,
          url: `${process.env.BASE_URL}supplier/delete/${id}`,
          headers: {
    ...
    <main className="flex min-h-screen flex-col items-center justify-between py-24 px-10 w-full">
      <div className="relative overflow-x-auto w-full rounded-md bg-gray-50   shadow-lg">
        ...
        <div className="options p-2 py-3 flex justify-between w-full items-center">
          <p>Supplier</p>
          <Link href={`/supplier/tambahsupplier/`}>
            <button className="bg-sky-700 py-1 px-3 rounded-md text-white">Tambah Supplier</button>{" "}
          </Link>
        </div>
              ...
              {listSupplier.length > 0 ? (
                listSupplier.map((supplier, i) => (
                  <tr key={i} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td className="px-6 py-4 border">{supplier.namaSupplier}</td>
                    <td className="px-6 py-4 border">{supplier.alamat}</td>

                    <td className="px-6 py-4 border"> {supplier.noTelp}</td>
                    <td className="px-6 py-4 border flex gap-0.5 justify-center">
                      <button className="bg-red-500 text-slate-200 p-2 rounded-md" onClick={() => deleteSupplier(supplier.id, supplier.namaSupplier)}>
                        Hapus
                      </button>
                      <Link href={`supplier/editsupplier/${supplier.id}`}>
