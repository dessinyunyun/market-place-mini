  ...
  const submitEditBarang = async () => {
    ...
    try {
      const config = {
        ...
        url: `${process.env.BASE_URL}barang/update/${id}`,
        headers: {
    ...
    const getBarang = async () => {
      ...
      try {
        const config = {
          method: "get",
          maxBodyLength: Infinity,
          url: `${process.env.BASE_URL}barang/find-by-id/${id}`,
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
        ...
        <div className="body-login text-slate-600 flex flex-col justify-center px-3 w-full items-center gap-2 my-4">
          ...
          <div className="namaSupplier w-full flex items-center gap-3">
            ...
            <input
              type="text"
              id="namaSupplier"
              value={barangData?.supplier?.namaSupplier}
              name="namaSupplier"
          ...
          <div class="button-option w-full flex justify-between pt-2 border-t">
            ...
            <button className="bg-blue-500 text-white font-light w-1/6 mt-3 text-sm rounded-md py-1.5 px-2" onClick={submitEditBarang}>
              Simpan
            </button>
          </div>
        </div>
